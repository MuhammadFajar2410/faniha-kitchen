...
@section('title','Faniha | All Products')

@section('content')
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h5 class="m-0 font-weight-bold text-muted">All Products</h5>
        </div>
        <div class="card-body">
                    ...
                    <tbody>
                        @foreach ($products as $product )
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('edit-product-admin', ['slug' => $product->slug]) }}" class="btn btn-warning btn-sm" style="height:30px; width:30px;border-radius:50%" ><i class="fas fa-edit"></i></a>
                                    <form action="{{ route('delete-product-admin', ['slug' => $product->slug]) }}" method="POST" id="form-delete" style="display:inline;">
                                        @method('DELETE')
                                        @csrf
    ...
    @include('backend.products-admin.modals')
@endsection
@push('style')
