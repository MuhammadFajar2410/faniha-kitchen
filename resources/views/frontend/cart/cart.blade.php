							...
							<form action="{{route('cart-update')}}" method="POST">
								...
								@if($carts)
									@foreach($carts as $key=>$cart)
										<tr>
											<td class="image" data-title="No"><img src="{{asset("backend/uploads/image/products/" .$cart->products->img)}}" alt="{{$cart->products->product_name}}"></td>
											<td class="product-des" data-title="Description">
												<p class="product-name"><a href="{{route('product-detail',$cart->products['slug'])}}" target="_blank">{{$cart->products['product_name']}}</a></p>
							...
							<div class="col-lg-4 col-md-7 col-12">
								<div class="right">
									{{-- <ul>
										...
										<li class="coupon_price" data-price="{{Session::get('coupon')['value']}}">You Save<span>{{'Rp ' . number_format(Session::get('coupon')['value'], 0, ',', '.')}}</span></li>
										@endif
										@php
										...
										@endphp
										@if(session()->has('coupon'))
											<li class="last" id="order_total_price">You Pay<span>{{'Rp ' . number_format($total_amount, 0, ',', '.')}}</span></li>
										@else
											<li class="last" id="order_total_price">You Pay<span>{{'Rp ' . number_format($total_amount, 0, ',', '.')}}</span></li>
										@endif
									</ul> --}}
